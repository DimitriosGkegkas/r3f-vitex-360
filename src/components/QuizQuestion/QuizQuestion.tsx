...
export const QuizQuestion: React.FC<QuizQuestionProps> = ({
  ...
  const [timeLeft, setTimeLeft] = useState(autoNextDelay / 1000);

  // Auto-advance timer effect
  useEffect(() => {
    if (showResult && onAutoNext) {
      ...
      setTimeLeft(autoNextDelay / 1000);
    }
  }, [showResult, onAutoNext, autoNextDelay]);
  ...
  const getOptionBackground = (index: number) => {
    ...
    if (index === correctAnswer) {
      return 'rgba(14.91, 196.74, 20.97, 0.20)';
    }
    
    if (index === selectedAnswer && index !== correctAnswer) {
      return 'rgba(196.74, 14.91, 14.91, 0.20)';
    }
    ...
    <div className="quiz-question">
      <div className="question-header">
        <div className="question-counter">
          Ερώτηση {currentQuestion} από {totalQuestions}
        </div>
      ...
      <div className="options-container">
        {question.options.map((option, index) => (
          <div
            ...
            style={{ background: getOptionBackground(index) }}
          ...
          {onAutoNext && (
            <div className="countdown-timer">
              <FilledButton
                ...
                loadingPercentage={(1 - (timeLeft) / (autoNextDelay / 1000)) * 100}
                onClick={onAutoNext}
                variant="primary"
                icon={
